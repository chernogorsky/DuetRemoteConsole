...
#include "duetbuddy_wifi.h"
#include "duetbuddy_web_server.h"

String sketch_version = "v0.0.1";



bool ready_wifi = false;
...
void loop() {
  initial_setup();
//  String wait_text = "";
//  wait_for_action("Connecting to WiFi ...", is_wifi_ready);
  draw_line("DoNothing");
  ez.buttons.wait("reset");
  ready_wifi = false;
...
/*
 * No config is present, or presed reset config
 */
void initial_setup(){
  fs_setup();
  wifi_setup_ap();
  web_server_setup();
  wait_for_action("Connect to http://"+wifi_get_ip(), is_wifi_ready);
}
...
void wait_for_action(String wait_text, bool (*check_func)()){
  wait_for_action_draw(wait_text);

  String btnpressed;

  while (!check_func()) {
    btnpressed = ez.buttons.poll();
//    if (btnpressed == "select") break;
    if (btnpressed == "setup") { 
      menu_setup(); 
      wait_for_action_draw(wait_text);
    };
...
void draw_line(String line_text){
//  M5.Lcd.setTextDatum(TR_DATUM);
  M5.Lcd.clear(BLACK);
  ez.header.show(sketch_version);
  m5.lcd.setTextColor(TFT_RED);
  ez.setFont(&FreeSansBold9pt7b);
  M5.Lcd.setTextDatum(CC_DATUM);
  M5.Lcd.drawString(line_text, (int)(M5.Lcd.width()/2), (int)((M5.Lcd.height()/2)-16));
...
void wait_for_action_draw(String wait_text){
  draw_line(wait_text);
  ez.buttons.show(" # # setup");
}

void menu_setup(void){
  ezMenu mainmenu("Setup");
  mainmenu.txtSmall();
  
  mainmenu.buttons("up # down # select ");
  
  mainmenu.addItem("WiFi settings", mainmenu_nothing);
  mainmenu.addItem("Printer settings", mainmenu_nothing);
  mainmenu.addItem("Reset config", mainmenu_nothing);
  mainmenu.addItem("Save config", mainmenu_nothing);
